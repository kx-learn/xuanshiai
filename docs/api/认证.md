...
签署 `safety_pledge` 只记录协议版本留痕，不直接产生单身承诺认证。用户还需通过资质认证接口上传手写签名，并在后台审核通过后才记录单身承诺完成状态；详见[资质认证接口](资质认证.md)。
...
签署当前版本的 `safety_pledge` 只完成协议留痕。单身承诺认证须通过 `/users/me/certifications/single-pledge` 上传手写签名并等待人工审核通过。
